...
export const Header = () => {
  ...
  return (
    <div className='header'>
        ...
        <div onClick={()=> setIsVisibleMenu(!is_visible_menu)} style={{backgroundImage: `url(${JSON.parse(profile).photoURL})`}} className='profile header__profile'>
            {is_visible_menu &&
            <div ref={menu} className='menu profile__menu'>
                <div onClick={()=> dispatch(UserLogout({navigate: navigate}))} className='logout menu__logout'>Выйти</div>
            </div>}
        </div>
