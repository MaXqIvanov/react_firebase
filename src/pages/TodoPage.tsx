...
import { AiOutlinePlus, AiOutlineFileAdd, AiOutlineQuestion } from 'react-icons/ai';
...
import {
  ...
  GetTodo,
  uploadFile,
} from '../store/reducers/todo/ActionTodo';
      ...
      {todos?.length > 0 &&
        todos.map((todo: TTodo, index: number) => (
          <div
            ...
            key={todo.id}
            className="todo todos__todo"
          >
            <div className="img todo__img">
              {countImage?.length > 0 &&
                countImage.map((elem: number, index: number) => (
                  <div key={index} className="spring_img">
                    <SiSpringCreators />
                  </div>
                ))}
            </div>
            <div className="todo__name">
              <input
                onBlur={() => dispatch(ChangeTodo(todo))}
                onChange={(e) => dispatch(changeCurrentTodo({ name: e.target.value }))}
                value={todo!.name}
              />
              <div
                onClick={() => {
            ...
            <div className="todo_wrapper">
              {todo.tasks?.length > 0 &&
                todo.tasks.map((task: TTask, index_task: number) => (
                  <>
                    <div
                      key={task.id}
                      onClick={() => dispatch(setCurrentTask({ index_task, index }))}
                      className="task todo__task"
                    >
                      <input
                        onChange={(e) => {
                      ...
                      <div className="name task__name">
                        <input
                          style={{
                            textDecoration:
                              task.complete || currentDate > new Date(task.finish_date).getTime()
                                ? 'line-through'
                                : 'none',
                          }}
                          onBlur={() => dispatch(ChangeTodo(todo))}
                          value={task.name}
                          onChange={(e) => dispatch(changeCurrentTask({ name: e.target.value }))}
                        />
                        <div
                      ...
                      {task.file?.length > 1 ? (
                        ...
                        <label
                          title="Прикрепить файл к задаче"
                          htmlFor={`${task.id}`}
                          className="load_file"
                        >
                          <AiOutlineFileAdd />
                          <input
                            ...
                            type={'file'}
                            id={`${task.id}`}
                            style={{ opacity: '0', maxHeight: '1px', maxWidth: '1px' }}
                          />
                        </label>
                      )}
                      ...
                      <div
                        onClick={() => setIsVisibleDescription(!isVisibleDescription)}
                        title="Показать описание задачи"
                        className="task__description"
                      >
                        <AiOutlineQuestion />
                      </div>
                    </div>
                    {isVisibleDescription && index_task === current_index_task && (
                      <div ref={text_area}>
                        <textarea
                          onMouseLeave={() => dispatch(ChangeTodo(todo))}
                          value={task.description}
                          onChange={(e) =>
                            dispatch(changeCurrentTask({ description: e.target.value }))
                          }
            ...
            <div onClick={() => dispatch(DeleteTodo(todo))} className="delete_btn">
              <RiDeleteBin6Line />
            </div>
          </div>
        ))}
      <div className="todo todos__todo">
        ...
        <div className="todo__name">
          <input
            ...
            onChange={(e) => setCreateTodoInput(e.target.value)}
            placeholder="Название todo листа"
          />
        </div>
      </div>
    </div>
